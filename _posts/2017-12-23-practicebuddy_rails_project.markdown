...
PracticeBuddy is meant to serve as a sort of repository for a player’s constantly evolving musical vocabulary. It helps musicians keep track of all their licks—key details about each phrase, as well as dates when it should be practiced again—so that they can focus more on perfecting each lick, and worry less about forgetting the ones they haven’t used in a while.
...
You can also sign up via Facebook, which I implemented using the gems: `omniauth`, `omniauth-facebook`, and `dotenv`. 
...
Once a user has successfully signed up / signed in, the first view they are shown is the Home view. This page is meant to be a summary of that day’s practice schedule and shows the user which licks are most important for them to focus on.

To collect this information, the app uses class level scope methods to query relevant data and organize it into sections:
* *Licks of the Day*—rounds up all the `Lick` objects whose `scheduled_practice` (datetime) attribute matches the current date
...
`Lick` objects, and the various CRUD actions that can be performed on them, are really the main feature of the app, since that’s what the whole concept is based around. `Lick` resources are nested under `User`, as they are specific to each individual player.
...
An additional class, called `BackingTrack`, has been included so that users can easily access online tracks to apply their musical phrases to. Similar to the `Lick` class, the full suite of `BackingTrack` CRUD actions are available as nested resources under `User`.

Unlike the `Lick` class, however, users can access `backing_track#show` and `backing_track#index` actions in routes outside of the nested resources, as well. While licks are purposely specific to their individual user, I wanted `BackingTracks` to be more “communal”. By having nested routes and routes that aren’t nested, a user can see their personal `BackingTracks` (`/users/:id/backing_tracks`) or see all of the `BackingTracks` that have been created by any user (`/backing_tracks`). Users can also add any `BackingTrack` to their licks, for use in future implementation of their phrases.
...
So, that’s *PracticeBuddy*! A simple tool to help musicians keep track of all the musical phrases they have learned in the past and be sure that each one is ready to go for their next gig or jam session. Time to practice some chops! And after that, I think I’ll get going on the JavaScript section.
